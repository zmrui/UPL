...
def set_cc(CCA):
    setcmd="echo 'net.ipv4.tcp_congestion_control=%s' >> /etc/sysctl.conf"%CCA
    checkcmd="cat /proc/sys/net/ipv4/tcp_congestion_control"
...
def onetest(cca,buffer1,buffer2,bandwidth1,bandwidth2,latency1,latency2,cs,sc,fc,fs):
    ...
    print(Server_result)
    stat = h1.cmd("netstat -s -t")
    fc.write(stat)
    fc.flush()
    stat = h2.cmd("netstat -s -t")
    fs.write(stat)
    fs.flush()
    net.stop()
...
def Figure():
    SIZE_C2S = [1]
    SIZE_S2C = [1000000]
    TCPCCAs=["cubic","bbr", "cdg", "highspeed", "htcp", "hybla", "illinois", "nv", "scalable", "vegas", "veno", "westwood", "yeah"]
    BUFFER1=[1.0]
    BUFFER2=[0.25, 0.5, 1, 2, 4, 8, 16, 32]
    BANDWIDTH1=[100]
    BANDWIDTH2=[10]
    LATENCY1=[1]
    LATENCY2=[50]
